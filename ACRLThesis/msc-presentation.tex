...
\begin{frame}{The Model Composes}
	...
	\end{block}
	\vfill
	The executor imposes \textbf{no} coordination policy. The model decides \emph{where} synchronization is needed and \emph{which} steps can run in parallel, guided only by general rules in the prompt, never an explicit script.
\end{frame}
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			\begin{itemize}
				...
				\item Same primitives also \textbf{parallelize} independent pick-and-places automatically (4/6 backends).
			\end{itemize}
		\end{column}
		\begin{column}{0.5\textwidth}
			\centering
	...
	\begin{columns}[c]
		\begin{column}{0.5\textwidth}
			...
			\begin{itemize}\itemsep2pt
				...
				\item \textbf{VGN +35 pp} (33 pooled): center-of-object grasp, not predicted orientation.
...
\begin{frame}{Five Contributions}
	\begin{enumerate}\itemsep4pt
		...
		\item \textbf{Runtime-composed coordination}: the model builds handoffs and parallelism from \texttt{signal}/\texttt{wait} and parallel groups over a \emph{shared} object.
...
\begin{frame}[noframenumbering]{Backup: Prompt-Driven Parallelism}
	\begin{itemize}
		...
		\item Manipulation phases run concurrently; detection kept sequential.
	\end{itemize}
	\vfill
