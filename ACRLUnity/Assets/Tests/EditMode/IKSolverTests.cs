...
namespace Tests.EditMode
{
    ...
    public class IKSolverTests
    {
        ...
        /// </summary>
        private static void AssertDeltasAreFinite(
            MathNet.Numerics.LinearAlgebra.Vector<double> deltas,
            string context = ""
        )
        {
            ...
            {
                Assert.IsFalse(
                    double.IsNaN(deltas[i]),
                    $"Joint {i} delta is NaN{(context.Length > 0 ? $" ({context})" : "")}"
                );
                Assert.IsFalse(
                    double.IsInfinity(deltas[i]),
        ...
        public void ComputeJointDeltas_Produces_Valid_Vector_Dimensions()
        {
            ...
            var currentState = new IKState(position: Vector3.zero, rotation: Quaternion.identity);
            var targetState = new IKState(
