...
def test_b9_get_tasks_returns_list():
    from benchmarks.cases import B9RagAblation
    from benchmarks.Config import BenchmarkConfig

    tasks = B9RagAblation.get_tasks(BenchmarkConfig())
...
def test_b10_get_tasks_returns_list():
    from benchmarks.cases import B10ReflexionAblation
    from benchmarks.Config import BenchmarkConfig

    tasks = B10ReflexionAblation.get_tasks(BenchmarkConfig())
    assert isinstance(tasks, list)
    assert len(tasks) >= 3


def test_b11_get_tasks_returns_list():
    from benchmarks.cases import B11NegotiationAblation
    from benchmarks.Config import DualRobotConfig

    tasks = B11NegotiationAblation.get_tasks(DualRobotConfig())
    assert isinstance(tasks, list)
    assert len(tasks) >= 2
    assert all(isinstance(t, str) for t in tasks)
...
def test_b12_get_tasks_returns_list():
    from benchmarks.cases import B12KgAblation
    from benchmarks.Config import BenchmarkConfig

    tasks = B12KgAblation.get_tasks(BenchmarkConfig())
    assert isinstance(tasks, list)
    assert len(tasks) >= 3
...
def test_b10_live_dispatches_to_send(monkeypatch):
    ...
    def fake_send(self_or_payload, robot_id_or_cfg=None, cfg=None):
        ...
        if callable(getattr(robot_id_or_cfg, "robot_id", None)) or isinstance(
            robot_id_or_cfg, str
        ):
