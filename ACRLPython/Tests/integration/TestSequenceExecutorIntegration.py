...
from operations.Base import OperationResult, BasicOperation, OperationCategory
...
class TestSequenceExecutorVerification:
    ...
    def test_coordination_conflict_blocks_execution(
        self,
        mock_coord_verifier_class,
        mock_verifier_class,
        mock_registry,
        cleanup_world_state,
    ):
        ...
        mock_verifier = mock_verifier_class.return_value
        mock_verifier.verify_preconditions = Mock(
            return_value=VerificationResult(success=True)
        )
...
class TestSequenceExecutorEndToEnd:
    ...
    def test_e2e_detect_and_move(
        ...
        mock_registry.return_value.execute_operation_by_name = Mock(
            side_effect=execute_op_by_name
        )
    ...
    def test_e2e_detect_move_grip(
        ...
        mock_registry.return_value.get_operation_by_name = Mock(
            side_effect=lambda name: operations.get(name)
        )

        # Mock results
        results = {
            "detect_object_stereo": OperationResult.success_result(
                {"position": (0.3, 0.15, 0.1)}
            ),
            "move_to_coordinate": OperationResult.success_result(
                {"final_position": (0.3, 0.15, 0.1)}
            ),
            "control_gripper": OperationResult.success_result(
                {"gripper_state": "closed"}
            ),
        }

        mock_registry.return_value.execute_operation_by_name = Mock(
            side_effect=lambda name, **params: results.get(
                name, OperationResult.error_result("ERROR", "Unknown", [])
            )
        )
        mock_broadcaster.return_value.get_completion = Mock(return_value=None)
        ...
        commands = [
            ...
            {
                "operation": "move_to_coordinate",
                "params": {"robot_id": "Robot1", "x": 0.3, "y": 0.15, "z": 0.1},
            },
            {
                "operation": "control_gripper",
    ...
    def test_e2e_multi_robot_coordination(
        ...
        commands = [
            ...
            {
                "operation": "move_to_coordinate",
                "params": {"robot_id": "Robot2", "x": 0.3, "y": 0.2, "z": 0.1},
            },
        ]
...
class TestSequenceExecutorErrorPropagation:
    ...
    def test_e2e_detection_fails(self, mock_registry, cleanup_world_state):
        ...
        mock_registry.return_value.get_operation_by_name = Mock(
            side_effect=lambda name: (
                detect_op if name == "detect_object_stereo" else move_op
            )
        )
        ...
        mock_registry.return_value.execute_operation_by_name = Mock(
            return_value=OperationResult.error_result(
                "NO_OBJECTS", "No objects detected", ["Adjust camera", "Check lighting"]
            )
        )
    ...
    def test_e2e_gripper_fails(self, mock_registry, cleanup_world_state):
        ...
        mock_registry.return_value.execute_operation_by_name = Mock(
            return_value=OperationResult.error_result(
                ...
                ["Reset gripper", "Check for obstruction"],
            )
        )
    ...
    def test_e2e_coordination_conflict(
        ...
        def execute_with_conflict(name, **params):
            ...
            if call_count[0] == 1:
                ...
                return OperationResult.error_result(
                    "COLLISION",
                    "Path blocked by Robot1",
                    ["Wait for Robot1", "Use different path"],
                )

        mock_registry.return_value.execute_operation_by_name = Mock(
            side_effect=execute_with_conflict
        )
        mock_broadcaster.return_value.get_completion = Mock(return_value=None)
        ...
        commands = [
            ...
            {
                "operation": "move_to_coordinate",
                "params": {"robot_id": "Robot2", "x": 0.3, "y": 0.0, "z": 0.1},
            },
        ]
...
class TestComplexSequences:
    ...
    def test_handoff_between_robots(
        ...
        commands = [
            ...
            {
                "operation": "move_to_coordinate",
                "params": {"robot_id": "Robot1", "x": 0.0, "y": 0.0, "z": 0.2},
            },
            {
                "operation": "move_to_coordinate",
                "params": {"robot_id": "Robot2", "x": 0.0, "y": 0.0, "z": 0.2},
            },
            {
                "operation": "control_gripper",
                "params": {"robot_id": "Robot1", "open_gripper": True},
            },
            {
                "operation": "control_gripper",
    ...
    def test_parallel_movements(
        ...
        commands = [
            ...
            {
                "operation": "move_to_coordinate",
                "params": {"robot_id": "Robot3", "x": 0.3, "y": 0.2, "z": 0.1},
            },
        ]
    ...
    def test_compound_command_sequence(
        ...
        results = {
            "detect_object_stereo": OperationResult.success_result(
                {"position": (0.3, 0.15, 0.1), "confidence": 0.95}
            ),
            "move_to_coordinate": OperationResult.success_result(
                {"final_position": (0.3, 0.15, 0.1)}
            ),
            ...
            "check_robot_status": OperationResult.success_result(
                {"is_moving": False, "position": (0.3, 0.15, 0.1)}
            ),
        }
        ...
        commands = [
            ...
            {
                "operation": "move_to_coordinate",
                "params": {"robot_id": "Robot1", "x": 0.0, "y": 0.0, "z": 0.3},
            },
            {
                "operation": "control_gripper",
                "params": {"robot_id": "Robot1", "open_gripper": True},
...
class TestVariablePassingIntegration:
    ...
    def test_variable_capture_and_use(
        ...
        detect_result = OperationResult.success_result(
            {"position": {"x": 0.35, "y": 0.18, "z": 0.12}}
        )
        move_result = OperationResult.success_result(
            {"final_position": (0.35, 0.18, 0.12)}
        )
        ...
        def execute_with_var(name, **params):
            ...
            if call_count[0] == 1:
                ...
                assert "position" in params or (
                    "x" in params and "y" in params and "z" in params
                )
                return move_result

        mock_registry.return_value.execute_operation_by_name = Mock(
            side_effect=execute_with_var
        )
        mock_broadcaster.return_value.get_completion = Mock(return_value=None)
    ...
    def test_multiple_variable_substitution(
        ...
        mock_registry.return_value.get_operation_by_name = Mock(
            side_effect=lambda name: (
                status_op if name == "check_robot_status" else move_op
            )
        )

        status_result = OperationResult.success_result(
            {"position": (0.1, 0.2, 0.3), "speed": 1.5}
        )
        move_result = OperationResult.success_result(
            {"final_position": (0.1, 0.2, 0.3)}
        )

        mock_registry.return_value.execute_operation_by_name = Mock(
            side_effect=lambda name, **params: (
                status_result if name == "check_robot_status" else move_result
            )
        )
        mock_broadcaster.return_value.get_completion = Mock(return_value=None)
        ...
        commands = [
            {
                ...
                "capture_var": "robot2_state",
            },
            {
                "operation": "move_to_coordinate",
                "params": {"robot_id": "Robot1", "position": "$robot2_state.position"},
            },
        ]
...
class TestSequenceExecutorPerformance:
    ...
    def test_rapid_consecutive_commands(
        ...
        commands = [
            {
                "operation": "control_gripper",
                "params": {"robot_id": "Robot1", "open_gripper": i % 2 == 0},
            }
            for i in range(10)
        ]
...
class TestSequenceExecutorEdgeCases:
    ...
    def test_partial_sequence_failure_recovery(
        ...
        def execute_with_failure(name, **params):
            ...
            if call_count[0] == 1:
                ...
                return OperationResult.error_result(
                    "COLLISION",
                    "Obstacle detected",
                    ["Clear obstacle", "Use different path"],
                )
            else:
        ...
        mock_registry.return_value.execute_operation_by_name = Mock(
            side_effect=execute_with_failure
        )
        mock_broadcaster.return_value.get_completion = Mock(return_value=None)
        ...
        commands = [
            {
                "operation": "move_to_coordinate",
                "params": {"robot_id": "Robot1", "x": 0.1, "y": 0.0, "z": 0.1},
            },
            {
                "operation": "move_to_coordinate",
                "params": {"robot_id": "Robot1", "x": 0.2, "y": 0.0, "z": 0.1},
            },
            {
                "operation": "move_to_coordinate",
                "params": {"robot_id": "Robot1", "x": 0.3, "y": 0.0, "z": 0.1},
    ...
    def test_variable_resolution_dotted_notation(self, cleanup_world_state):
        ...
        executor.set_variable(
            "target", {"x": 0.197, "y": 0.021, "z": 0.189, "color": "blue"}
        )
