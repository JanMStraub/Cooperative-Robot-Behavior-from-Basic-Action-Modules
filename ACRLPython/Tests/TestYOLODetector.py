...
class TestYOLODetectorInitialization:
    ...
    def test_yolo_detector_init_with_custom_model(self, mock_yolo_class):
        ...
        # Should load custom model
        mock_yolo_class.assert_called_once_with(custom_model_path)
...
class TestYOLODetection:
    ...
    @pytest.mark.skipif(not YOLO_AVAILABLE, reason="YOLO not available")
    @patch('vision.YOLODetector.YOLO')
    def test_detect_cubes_empty_image(self, mock_yolo_class):
    ...
    @pytest.mark.skipif(not YOLO_AVAILABLE, reason="YOLO not available")
    @patch('vision.YOLODetector.YOLO')
    def test_detect_cubes_with_detections(self, mock_yolo_class):
        ...
        mock_boxes = Mock()
        mock_boxes.xyxy = [np.array([100, 200, 150, 260])]  # x1, y1, x2, y2
        mock_boxes.cls = [np.array(0)]  # class ID 0 (red)
        mock_boxes.conf = [np.array(0.85)]  # confidence
        ...
        result = detector.detect_cubes(test_image, camera_id="test")
    ...
    @pytest.mark.skipif(not YOLO_AVAILABLE, reason="YOLO not available")
    @patch('vision.YOLODetector.YOLO')
    def test_detect_cubes_filters_by_area(self, mock_yolo_class):
        ...
        mock_boxes = Mock()
        mock_boxes.xyxy = [np.array([100, 200, 102, 202])]  # 2x2 pixels (too small)
    ...
    @pytest.mark.skipif(not YOLO_AVAILABLE, reason="YOLO not available")
    @patch('vision.YOLODetector.YOLO')
    def test_detect_cubes_multiple_detections(self, mock_yolo_class):
        ...
        mock_boxes = Mock()
        mock_boxes.xyxy = [
            np.array([100, 200, 150, 260]),  # Red cube
            np.array([300, 150, 360, 220]),  # Blue cube
        ]
        mock_boxes.cls = [np.array(0), np.array(1)]  # class IDs
        mock_boxes.conf = [np.array(0.85), np.array(0.92)]  # confidences
        ...
        # Check first detection (red)
        det1 = result.detections[0]
        assert det1.color == "red"
        assert det1.confidence == 0.85

        # Check second detection (blue)
        det2 = result.detections[1]
        assert det2.color == "blue"
