...
┌─────────────────┐       TCP (Port 5011)      ┌─────────────────┐
│ RAG Server      │ <─────────────────────────> │ Unity           │
│ (RunRAGServer)  │    Natural Language Queries │ (RAGClient.cs)  │
└─────────────────┘                             └─────────────────┘
```
...
### 4. Start RAG Server for Unity

```bash
# Start RAG server (port 5011)
python -m LLMCommunication.orchestrators.RunRAGServer

# Rebuild index on startup
python -m LLMCommunication.orchestrators.RunRAGServer --rebuild-index

# Test mode (show sample queries)
python -m LLMCommunication.orchestrators.RunRAGServer --test
```

**Unity Integration**:
```csharp
// In Unity, attach RAGClient component
var ragClient = gameObject.AddComponent<RAGClient>();

// Subscribe to results
ragClient.OnRagResultReceived += (result) => {
    Debug.Log($"Found {result.num_results} operations for: {result.query}");
    foreach (var op in result.operations) {
        Debug.Log($"  - {op.name} (score: {op.similarity_score})");
    }
};

// Query operations
ragClient.Query("move robot to position", topK: 3);

// Or use helper methods
ragClient.QueryNavigation("navigate to red cube");
ragClient.QueryManipulation("pick up object");
```

### 5. Find Similar Operations
