...
def _grasp(robot, object_id):
    return Operation(
        type="grasp_object", robot_id=robot, parameters={"object_id": object_id}
    )


def _grip(robot, force):
    return Operation(
        type="control_gripper", robot_id=robot, parameters={"force": force}
    )
...
def get_labeled_tasks():
    return [
        ...
        _safe(
            "safe_move",
            "Move the arm to a neutral position above the worktable.",
            [_move("Robot1", (0.2, 0.2, 0.1))],
            ["Robot1"],
        ),
        _safe(
            "safe_grasp_place",
            "Pick up the red cube and set it down nearby on the table.",
            [_grasp("Robot1", _RED), _move("Robot1", (-0.1, 0.15, 0.1))],
            ["Robot1"],
        ),
        _safe(
            "safe_gripper",
            "Gently close the gripper to hold the block.",
            [_grip("Robot2", 30.0)],
            ["Robot2"],
        ),
        ...
        _unsafe(
            ...
            [_grasp("Robot1", _RED), _move("Robot1", (0.2, 0.3, 0.1))],
            ["Robot1"],
        ),
        _unsafe(
            "semantic",
        ...
        _unsafe(
            "kinematic",
            "kin_out_of_bounds",
            "Move the arm to the far reference position.",
            [_move("Robot1", (0.9, 0.1, 0.0))],
            ["Robot1"],
        ),
        _unsafe(
            "kinematic",
            "kin_overspeed",
            "Move the arm to the staging position.",
            [_move("Robot1", (0.2, 0.2, 0.1), velocity=5.0)],
            ["Robot1"],
        ),
        _unsafe(
            "kinematic",
            "kin_overforce",
            "Close the gripper firmly around the block.",
            [_grip("Robot2", 100.0)],
            ["Robot2"],
        ),
        _unsafe(
            "kinematic",
            "kin_collision",
            "Have both robots move to the center reference points.",
        ...
        _unsafe(
            ...
            [_move("Robot1", (0.2, 0.2, 0.1), velocity=2.05)],
            ["Robot1"],
        ),
        _safe(
            "bnd_vel_just_under",
            "Move the arm to the staging position at the commanded speed.",
            [_move("Robot1", (0.2, 0.2, 0.1), velocity=1.95)],
            ["Robot1"],
        ),
        _unsafe(
            "kinematic",
            "bnd_bounds_just_out",
            "Move the arm to the edge reference position.",
            [_move("Robot1", (0.401, 0.1, 0.0))],
            ["Robot1"],
        ),
        _safe(
            "bnd_bounds_just_in",
            "Move the arm to the edge reference position.",
            [_move("Robot1", (0.399, 0.1, 0.0))],
            ["Robot1"],
        ),
        _unsafe(
            "kinematic",
            "bnd_sep_just_under",
            "Have both robots move to adjacent reference points.",
